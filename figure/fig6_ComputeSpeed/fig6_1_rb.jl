...
ntimesteps = [148.71266666666668, 339.1086666666667, 386.46, 465.49333333333334]
nterms = [3,21, 60, 126]
...
for idx in 1:4
    ...
    push!(computation_times, minimum(bench_sindy).time/1e9)
end
...
subplot1 = twinx(pp)
plot!(subplot1, [1,2,3,4], Float64.([fill(NaN, size(computation_times)) computation_times]),
        marker = (:circle,5),
        ...
        color = [:blue :red])
pp
savefig(pp, "raw/fig6_prod_time.svg")
...
plot!(subplot2, [1,2,3,4], Float64.([fill(NaN, size(ntimesteps)) ntimesteps]),
        ...
        color = [:blue :red])
pp2
savefig(pp2, "raw/fig6_eqn_time.svg")